...
class MealRepository {
  ...
  async findBySearch(search) {
    const mealsInfos = ["meals.id", "meals.title", "meals.description", "meals.price", "meals.image", "meals.category",];
    ...
    const searchByIngredients = await knex("meals")
      ...
      .innerJoin("ingredients", "meals_ingredients.ingredient_id", "ingredients.id")
      .whereLike("name", `%${search}%`);

    const searchResultWithoutDuplicateItems = [...new Set(searchByMealTitle.concat(searchByIngredients)),];
