...
class OrderRepository {
  ...
  async findMealsOfAnOrder(order_id) {
    const meals = await knex("meals")
      .select(["meals.id", "meals.title", "meals.price", "meals.image", "orders_meals.meal_amount",])
  ...
  async findAllOrders() {
    const orders = await knex("orders")
      .select(["orders.id", "orders.status", "orders.created_at", "orders_meals.id as order_meal_id", "meals.title", "orders_meals.meal_amount",])
