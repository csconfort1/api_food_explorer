...
class PlatesController{
    ...
    async index(request, response){
        ...
        if(title){
                ...
                plates
                :
                plates = await knex("plates").whereLike("plates.ingredients", `%${title}%`);
    ...
    async update(request, response){
        ...
        const [saveId] = await knex('favorites').where({id: plate_id});

        if(saveId){
            await knex('favorites').where({id: plate_id}).delete();
        ...
        if(!plate) {
            throw new AppError("Não foi possível encontrar o seu prato!");
        };
